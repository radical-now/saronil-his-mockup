...
(function() {
  ...
  function buildMockCharges() {
    ...
    charges.forEach(function(item) {
      if (labRadIcdMap[item.code]) {
        item.icd10Code = labRadIcdMap[item.code];
      }
    });
    return charges;
  }
  ...
  function setupHandlers(container) {
    ...
    window.cmDeactivate = async function(code) { var i=getItems().find(function(x){return x.code===code;}); if(!i) return; var reason; if(i.billingUseCount>0){reason=await customPrompt('Charge has been billed '+i.billingUseCount+' time(s). Enter deactivation reason:'); if(!reason) return; }else{if(!await customConfirm('Charge never billed. Hard delete or deactivate?')) {reason=await customPrompt('Enter deactivation reason:'); if(!reason) return;} else{window.state.chargeMaster.items=window.state.chargeMaster.items.filter(function(x){return x.code!==code;}); logAudit('Charge Deleted',code,i.name,'','','','Hard delete'); alert('\u2705 Deleted.'); renderShell(container); return;}} logAudit('Charge Deactivated',code,i.name,'Status',i.status,'Inactive',reason); i.status='Inactive'; alert('\u2705 Done.'); renderShell(container); };
    window.cmSuspend = async function(code) { var i=getItems().find(function(x){return x.code===code;}); if(!i) return; var r=await customPrompt('Enter suspension reason (required):'); if(!r) return; logAudit('Charge Suspended',code,i.name,'Status',i.status,'Suspended',r); i.status='Suspended'; alert('\u26A0\uFE0F '+i.name+' suspended. Resolve within 24 hours.'); renderShell(container); };
