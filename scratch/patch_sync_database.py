...
path_state = "/Users/home/Desktop/Saronil IHS/Updated HIS /assets/js/state.js"
path_patients = "/Users/home/Desktop/Saronil IHS/Updated HIS /assets/js/views/patientsView.js"

# 1. Update doctors database and seedState in state.js
with open(path_state, "r", encoding="utf-8") as f:
    src_state = f.read()

# Locate DOCTORS_DATABASE block start and end
start_doctors_idx = src_state.find("const DOCTORS_DATABASE = [")
end_doctors_idx = src_state.find("];", start_doctors_idx) + 2
...
src_state = src_state[:start_doctors_idx] + doctors_block + src_state[end_doctors_idx:]

# Define Nurses and Staff Database
...
state_def_start = src_state.find("const state = {")
first_state_entry = src_state.find("  doctors: DOCTORS_DATABASE,", state_def_start)
src_state = src_state[:first_state_entry] + nurses_staff_block + src_state[first_state_entry:]

# Now replace seedState function with a clean programmatic generation of exactly 100 patients
seed_state_start = src_state.find("function seedState() {")
seed_state_end = src_state.find("};\n\nstate.logBedMovement", seed_state_start) + 2
...
synchronized_seed_state = """function seedState() {
  ...
  const staticPatients = [
    ...
    },
    {
      ...
      primaryConsultant: "Dr. Fatima Sheikh", department: "ICU",
      payer: "Self Pay", payerType: "Cash", preAuthStatus: "—",
    ...
    },
    {
      ...
      primaryConsultant: "Dr. Ramesh Iyer", department: "General Medicine",
      payer: "Star Health", payerType: "TPA/Insurance", preAuthStatus: "LOA ✓",
    ...
    },
    {
      ...
      primaryConsultant: "Dr. Mehta", department: "Cardiology",
    ...
    },
    {
      ...
      primaryConsultant: "Dr. Ramesh Iyer", department: "Paediatrics",
      payer: "Cash Tariff", payerType: "Cash", preAuthStatus: "—",
  ...
  const depts = ["General Medicine", "Pediatrics", "Psychiatry", "Cardiology", "Neurology", "Gynecology & Obs", "General Surgery", "Dermatology", "Oncology"];
  const payers = ["Self Pay", "Star Health", "HDFC ERGO", "New India Assurance", "United India", "ECHS", "CGHS", "PMJAY", "ESI", "Corporate"];

  const distribution = [];
  // Target distribution for 85 patients:
  // - 15 IPD (Admitted/Post-Op)
  // - 30 OPD (Scheduled/Checked In/In Consultation)
  // - 10 Daycare (Under Observation/Day Care)
  // - 5 Emergency (Emergency/In Triage)
  // - 25 Discharged (Discharged/Registered)
  ...
  for (let idx = 0; idx < distribution.length; idx++) {
    ...
    if (config.type === "IPD" && config.status !== "Discharged") {
      ...
      // Also register an IPD admission record
    ...
    if (config.type === "OPD") {
      // Seed OPD clinical appointments
...
src_state = src_state[:seed_state_start] + synchronized_seed_state + src_state[seed_state_end:]

with open(path_state, "w", encoding="utf-8") as f:
    f.write(src_state)

# 2. Update patientsView.js to reuse state.patients
with open(path_patients, "r", encoding="utf-8") as f:
    src_patients = f.read()

old_generator_def_start = src_patients.find("function ensure307PatientsExist() {")
old_generator_def_end = src_patients.find("}\n\nfunction randomRange", old_generator_def_start) + 2

new_generator_def = """function ensure307PatientsExist() {
...
src_patients = src_patients[:old_generator_def_start] + new_generator_def + src_patients[old_generator_def_end:]

with open(path_patients, "w", encoding="utf-8") as f:
    f.write(src_patients)

print("SUCCESS: 100 patients, 20 doctors, 10 nurses, and 10 staff fully synced across modules.")
