...
# Locate start of p360Styles
start_marker = "  // Styling block\n  const p360Styles = `"
start_idx = src.find(start_marker)

# Locate end of draw360
end_marker = "  // Interactive controllers on window\n  window.prSwitchRole ="
end_idx = src.find(end_marker)

if start_idx == -1 or end_idx == -1:
    print("ERROR: Could not find start or end markers for layout replacement.")
    exit(1)

NEW_LAYOUT_CODE = """  // Styling block
  const p360Styles = `
    <style>
      .p360-wrap {
  ...
  function draw360() {
    ...
    // Quick Actions Row templates based on Role (horizontal inline format)
        ...
        <div class="p360-card" style="display:flex; flex-direction:column; gap:10px;">
          <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:16px;">
            ...
            <div style="display:flex; align-items:center; gap:12px;">
              <div class="pr-avatar" style="width:40px; height:40px; font-size:14px; font-weight:700; background:var(--primary); color:#ffffff;">LD</div>
              <div>
            ...
            <div style="display:flex; align-items:center; gap:6px;">
              ...
              <span class="flag-chip" style="background:var(--bg-surface-elevated, #f1f5f9); color:var(--text-muted); font-size:10px; margin:0;">No Known Allergy</span>
            </div>
          </div>
...
patched_src = src[:start_idx] + NEW_LAYOUT_CODE + src[end_idx:]

with open(path, "w", encoding="utf-8") as f:
    f.write(patched_src)

print("SUCCESS: Patient 360 layout design overhauled.")
"""

with open("/Users/home/Desktop/Saronil IHS/Updated HIS /scratch/patch_patient_360_layout.py", "w", encoding="utf-8") as f:
    f.write(NEW_LAYOUT_CODE)

print("SUCCESS: Patcher written successfully.")
